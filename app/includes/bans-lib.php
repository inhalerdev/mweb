...
function mineacle_epoch_seconds(mixed $value): int {
    ...
    // LiteBans stores BIGINT timestamps. Most modern LiteBans data is milliseconds.
    // Some imports may be seconds. Handle both without converting normal seconds incorrectly.
...
function mineacle_format_date(mixed $value): string {
    ...
    // 2038-style values are usually a permanent/sentinel value accidentally appearing as a ban date.
    // Do not show fake future dates as the ban date.
    if ($seconds > time() + 31536000) {
        return 'Unknown';
    }
