...
let currentPage = 1;
let currentRows = [];
let currentPagination = { page: 1, per_page: 25, total: 0, total_pages: 1, has_prev: false, has_next: false };

function escapeHtml(value) {
  return String(value ?? "").replace(/[&<>"']/g, (char) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "\"": "&quot;", "'": "&#039;" }[char]));
}

function badgeClass(ban) { return ban.status_type || "active"; }

function actionButton(ban) {
  if (ban.ipban) return `<button class="btn soft disabled" type="button" disabled>No Action</button>`;
  if (ban.can_pay) return `<a class="btn red" href="${escapeHtml(ban.unban_url)}">${escapeHtml(ban.price)} Unban</a>`;
...
function renderBans(rows) {
  ...
  if (!currentRows.length) {
    banList.innerHTML = `<div class="empty compact-state"><strong>No matching active bans found</strong><span>Check the username spelling or clear the search</span></div>`;
  ...
  banList.innerHTML = currentRows.map((ban) => `
    <article class="ban-row">
      ...
      <div class="ban-reason">${escapeHtml(ban.reason)}<div class="ban-meta">${escapeHtml(ban.type)} • ${escapeHtml(ban.duration)}</div></div>
      <div class="ban-status"><span class="badge ${escapeHtml(badgeClass(ban))}">${escapeHtml(ban.status)}</span></div>
...
async function readJson(response) {
  ...
  try { return JSON.parse(text); } catch { throw new Error("Invalid API response"); }
}
...
async function loadBans(page = currentPage) {
  ...
  try {
    ...
    if (!response.ok || !payload.success) throw new Error(payload.error || "Unable to load bans right now");
    ...
    banList.innerHTML = `<div class="error compact-state"><strong>Unable to load bans right now</strong></div>`;
    if (banCount) banCount.textContent = "0 shown";
    renderPagination();
  }
}
...
function openBanInfo(id) {
  ...
  if (ban.ipban) {
    ...
    actions.innerHTML = `<a class="btn red" href="${escapeHtml(ban.unban_url)}">${escapeHtml(ban.price)} Pay to be unbanned</a><a class="btn soft" href="${escapeHtml(ban.discord)}" target="_blank" rel="noopener">Contact Discord</a>`;
...
if (banSearch) {
  let timer = null;
  const runSearch = () => { currentPage = 1; updateClearButton(); loadBans(1); };
  banSearch.addEventListener("input", () => { clearTimeout(timer); timer = setTimeout(runSearch, 180); updateClearButton(); });
  banSearch.addEventListener("keydown", (event) => {
    if (event.key === "Enter") { event.preventDefault(); clearTimeout(timer); runSearch(); }
    if (event.key === "Escape") { banSearch.value = ""; clearTimeout(timer); runSearch(); }
  });
  banSearch.addEventListener("search", () => { clearTimeout(timer); runSearch(); });
  updateClearButton();
}
...
if (prevPageButton) prevPageButton.addEventListener("click", () => { if (currentPagination.has_prev) loadBans(currentPagination.page - 1); });
if (nextPageButton) nextPageButton.addEventListener("click", () => { if (currentPagination.has_next) loadBans(currentPagination.page + 1); });

document.querySelectorAll("[data-close-modal]").forEach((button) => button.addEventListener("click", closeModal));
document.addEventListener("keydown", (event) => { if (event.key === "Escape") closeModal(); });
if (banModal) banModal.addEventListener("click", (event) => { if (event.target === banModal) closeModal(); });
